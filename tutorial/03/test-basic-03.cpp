...
void test_threadpool_2(UThreadPoolPtr tp)
{
    ...
    taskGroup.addTask([i, j] {
        int result = i + j;
        NAO_SLEEP_MILLISECOND(1000)
        NAO_ECHO("sleep for 1 second, [%d] + [%d] = [%d], run success.", i, j, result);
    });
     ...
     * 并且在status中提示超时信息
     * */
    NStatus status = tp->submit(taskGroup, 2500);
    NAO_ECHO("task group run status is [%d].", status.getCode());
}
...
int main()
{
    ...
    NAO_ECHO("======== test_threadpool_2 begin. ========");
    test_threadpool_2(pool.get());

    NAO_ECHO("======== test_threadpool_3 begin. ========");
    test_threadpool_3(pool.get());
    return 0;
}