 ...
 * @Version      : 0.0.1
 * @LastEditors  : naonao
 * @LastEditTime : 2024-11-26 14:48:19
 **/
...
public:
     ...
     * @param dst
     */
    static NVoid get_histogram(const cv::Mat& src, NInt* dst);
     ...
     */
    NInt threshold_default(std::vector<NInt> data);
     ...
     */
    NInt huang(std::vector<NInt> data);
     ...
     * @details 实现Huang的模糊阈值方法,使用香农熵函数（也可以使用Yager熵函数）
     */
    NInt huang2(std::vector<NInt> data);
     ...
     */
    NInt intermodes(std::vector<NInt> data);
     ...
     */
    NInt iso_data(std::vector<NInt> data);
     ...
     */
    NInt max_entropy(std::vector<NInt> data);
     ...
     * @details 阈值是灰度数据的平均值
     */
    NInt mean(std::vector<NInt> data);
    ...
    */
    NInt min_errorI(std::vector<NInt> data);
     ...
     */
    NInt minimum(std::vector<NInt> data);
     ...
     */
    NInt moments(std::vector<NInt> data);
     ...
     * @return
     */
    NInt otsu(std::vector<NInt> data);
    /**
     * @brief 百分比阈值法，默认50%
     * @param data
     * @param ptile
     * @return
     */
    NInt percentile(std::vector<NInt> data, NDouble ptile = 0.5);
     ...
     */
    NInt renyi_entropy(std::vector<NInt> data);
     ...
     */
    NInt shanbhag(std::vector<NInt> data);
     ...
     * @details 三角形算法是一种几何方法，它无法判断数据是否偏向一侧或另一侧，但会假定直方图一端附近的最大峰值（众数）
     ...
     内搜索阈值。
    */
    NInt triangle(std::vector<NInt> data);
     ...
     */
    NInt sauvola(const cv::Mat& src, cv::Mat& dst, const NDouble& k = 0.1, const NInt& wnd_size = 7);
     ...
     * @return         返回阈值
     */
    NInt exec_threshold(cv::Mat& src, THRESHOLD_TYPE type, NInt doIblack = -1, NInt doIwhite = -1, NBool reset = false);
    ...
    NInt sample_otsu(cv::Mat img, NInt min_value = 0, NInt max_value = 255);
     ...
     **/
    cv::Mat unevenLightCompensate(cv::Mat image, NInt blockSize = 32);
    cv::Mat unevenLightCompensate_2(cv::Mat image, NInt blockSize = 32);
};
